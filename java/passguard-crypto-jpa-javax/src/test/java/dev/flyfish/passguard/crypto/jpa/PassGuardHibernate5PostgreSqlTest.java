...
class PassGuardHibernate5PostgreSqlTest {
    ...
    void storesOnlyCiphertextAndReadsPlaintext() throws Exception {
        ...
        BootstrapServiceRegistry bootstrap = new BootstrapServiceRegistryBuilder()
                .applyIntegrator(new PassGuardHibernate5Integrator(processor))
                .build();
        ...
        try (SessionFactory sessions = new MetadataSources(registry)
                ...
                .build()) {
            Credential saved = new Credential(1L, "hibernate5-secret");
            try (Session session = sessions.openSession()) {
                session.beginTransaction();
                session.persist(saved);
                session.getTransaction().commit();
            }
            assertEquals("hibernate5-secret", saved.password);
            ...
            try (Connection connection = DriverManager.getConnection(
                 ...
                 ResultSet result = statement.executeQuery(
                         "select password,password_index from pg_credential5 where id=1")) {
            ...
            try (Session session = sessions.openSession()) {
                ...
                assertEquals("hibernate5-secret", loaded.password);
                String index = blindIndexes.compute(
                        "hibernate5-secret", "index", "pg_credential5.password_index");
            ...
            try (Session session = sessions.openSession()) {
                ...
                updated.password = "hibernate5-rotated";
                session.getTransaction().commit();
            }
            assertEquals("hibernate5-rotated", updated.password);
            try (Connection connection = DriverManager.getConnection(
                    jdbcUrl(), username(), password());
                 Statement statement = connection.createStatement();
                 ResultSet result = statement.executeQuery(
                ...
                assertEquals(blindIndexes.compute(
                                "hibernate5-rotated", "index",
                                "pg_credential5.password_index"),
                        result.getString(2));
            }
        } finally {
    ...
    @Table(name = "pg_credential5")
    public static class Credential {
        @Id
        private Long id;

        @Encrypted(keyAlias = "data", context = "pg_credential5.password")
        ...
        @BlindIndex(
                source = "password",
                keyAlias = "index",
                context = "pg_credential5.password_index")
