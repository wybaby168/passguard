...
class PassGuardR2dbcPostgreSqlTest {
    ...
    void storesOnlyCiphertextAndReadsPlaintext() {
        ...
        template.getDatabaseClient().sql("drop table if exists pg_r2dbc3")
                .fetch().rowsUpdated().block();
        template.getDatabaseClient().sql(
                "create table pg_r2dbc3("
                        + "id bigint primary key,password varchar(2048) not null,"
                        + "password_index varchar(128))")
                .fetch().rowsUpdated().block();

        Credential saved = new Credential(1L, "r2dbc3-secret");
        template.insert(saved).block();
        assertEquals("r2dbc3-secret", saved.password);
        assertNull(saved.passwordIndex);

        String raw = template.getDatabaseClient()
                .sql("select password from pg_r2dbc3 where id=1")
        ...
        assertEquals("r2dbc3-secret", loaded.password);

        String index = blindIndexes.compute(
                "r2dbc3-secret", "index", "pg_r2dbc3.password_index");
        ...
        saved.password = "r2dbc3-rotated";
        template.update(saved).block();
        assertEquals("r2dbc3-rotated", saved.password);
        assertNull(saved.passwordIndex);
    ...
    public static class Credential {
        ...
        @Encrypted(keyAlias = "data", context = "pg_r2dbc3.password")
        private String password;

        @BlindIndex(
                source = "password",
                keyAlias = "index",
                context = "pg_r2dbc3.password_index")
